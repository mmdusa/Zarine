...
import { GREEN_TEA } from "../greenTeaData"; // same structure as TEA_BAGS

export default function GreenTeaDetailPage({ params }: { params: { slug: string } }) {
  const product = GREEN_TEA.find((p) => p.id === params.slug);
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-[#120906] via-[#0A0504] to-black text-white pb-20">
      ...
      <section className="max-w-7xl mx-auto px-6 pt-16 grid md:grid-cols-[minmax(0,2.4fr)_minmax(0,3fr)] gap-12">
        ...
        {/* 🟢 Left = image like Tea Bags */}
        <div className="rounded-3xl border border-[#d4af37]/25 bg-black/60 p-6 shadow-[0_20px_90px_rgba(0,0,0,.7)]">
          <div className="relative w-full aspect-[4/3]">
            <Image src={product.image} alt={product.name} fill className="object-contain drop-shadow-[0_0_40px_rgba(0,0,0,.8)]" />
          </div>
        </div>

        {/* 🟢 Right side = EXACT UI structure */}
        <div>
          <Link href="/products/tea/green" className="text-[11px] uppercase opacity-60 hover:opacity-100">← Back to Green Tea</Link>

          <h1 className="mt-2 text-4xl font-bold text-[#7CFFB5]">{product.name}</h1>

          {/* Pills exactly like Tea Bags */}
          ...
          {/* PRICE MODULE — IDENTICAL TEA BAG STYLE */}
          <div className="mt-8 rounded-3xl border border-[#d4af37]/40 bg-gradient-to-r from-[#24130d] via-[#2f1810] to-[#3c2213] p-6 flex items-center justify-between">
            <p className="text-sm opacity-85 max-w-md">
              Classic Lahijan green tea — clean taste, perfect for daily drinking.
            </p>
          ...
          {/* DESCRIPTION SECTION LIKE TEA BAGS */}
          <div className="mt-10 space-y-6 text-white/85 leading-relaxed">
            <div>
              <p className="font-bold text-[#7CFFB5] mb-1">Story</p>
              <p>{product.description}</p>
            </div>

            <div>
              <p className="font-bold text-[#7CFFB5] mb-1">Brewing & Serving</p>
              <p>Let steep for {product.brew}. Try with honey, mint or lemon.</p>
            </div>
          </div>
        </div>
      </section>
...
/* global copy — EXACT tea bag styling */
<style jsx global>{`
  .pill {
    padding: 5px 12px;
    border-radius: 999px;
