...
export default function BarberryProductPage({ params }: PageProps) {
  // ✅ FIX: unwrap params (Next.js 15 requirement)
  const { slug } = use(params);

  const product = ZERESHK_PRODUCTS.find((p) => p.id === slug);
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-[#120906] via-[#090304] to-black text-white pb-20">
      <section className="max-w-7xl mx-auto px-6 pt-16 grid md:grid-cols-[minmax(0,2.4fr)_minmax(0,3fr)] gap-12">
        ...
        <div>
          <Link
            href="/products/barberry"
            className="text-[11px] uppercase opacity-60 hover:opacity-100"
          >
            ← Back to Zereshk
          </Link>

          <h1 className="mt-2 text-4xl font-bold text-[#FF6B6B]">
            {product.name}
          </h1>
          ...
          <div className="mt-10 space-y-6 text-white/85 leading-relaxed">
            ...
            <div>
              ...
              <p>
                Add to rice (zereshk polo), salads, tea, jam, desserts or brew in
                warm water with saffron + honey.
              </p>
            </div>
          </div>
        </div>
